...
class CancelJoinTournamentView(APIView):
	...
	def post(self, request, *args, **kwargs):
		...
		try:
			...
			logging.info(f"Creator {tournament_to_cancel_join.creator.id} vs user_id {user_id}")
			if (tournament_to_cancel_join.creator.id == user_id):
				players_in_tournament = PlayerTournament.objects.filter(tournament=tournament_id)
				for player in players_in_tournament:
					player.delete()
